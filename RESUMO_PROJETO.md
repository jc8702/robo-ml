...
- **29/07/2026 - 17:51:** Implementada a arquitetura Serverless Nativa da Vercel (`api/config.ts` e `api/history.ts`). Como o diretório `dist/` fica no `.gitignore`, as funções serverless apontando para `dist/server.js` falhavam ao rotear a API na nuvem. A criação das rotas nativas em TypeScript dentro da pasta `api/` permite que a Vercel compile e sirva os endpoints diretamente. No frontend (`public/index.html`), adicionada a renderização síncrona instantânea (`selectedCategories = getAllSubCategoryIds(); renderCascadeTree()`), garantindo que todas as 80+ subcategorias apareçam marcadas na tela em **0 milissegundos** no momento em que a página é aberta.
