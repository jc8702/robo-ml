...
- **29/07/2026 - 18:26:** Solução dos 2 problemas reportados: 1) **Persistência da Sessão WhatsApp no Neon**: Implementadas as rotinas `restoreWaAuthFromDb()` e `saveWaAuthToDb()` em `src/whatsapp/client.ts`. Os arquivos de sessão da pasta `.wa-auth/` agora são automaticamente sincronizados com o banco Neon PostgreSQL (`app_settings`), fazendo o worker no Render manter a conexão ativa 24/7 sem solicitar novos códigos QR/Pairing. 2) **Painel Vercel Limpo**: Atualizados os manipuladores de botões e logs no `public/index.html` e `index.html` para operação estática nativa sem tentar realizar `fetch` em rotas inexistentes, eliminando todos os erros `Unexpected token '<'` no console.
