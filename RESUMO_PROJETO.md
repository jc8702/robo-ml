...
- **29/07/2026 - 16:42:** Preparação total para deploy de nuvem 24/7. Criado o módulo `src/db/index.ts` com o cliente do PostgreSQL (`pg`) para auto-inicializar as tabelas `sent_history` e `price_history` no **Neon PostgreSQL** quando `DATABASE_URL` estiver presente no `.env`. Criado o `Dockerfile` otimizado para a execução 24/7 com Playwright Chromium em servidores Linux (Railway/Render/VPS) e o arquivo `vercel.json` para o deploy do Dashboard na **Vercel**. O `.gitignore` foi atualizado com padrões de segurança estritos.
