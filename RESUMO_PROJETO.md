...
- **29/07/2026 - 18:20:** Erradicação total do erro 500 na Vercel. Removida a pasta `api/` e configurado o `vercel.json` com o builder nativo `@vercel/static` apontando diretamente para `public/index.html`. Com essa mudança, a Vercel atua 100% como uma CDN de hospedagem estática de ultra performance, eliminando a criação de qualquer Serverless Function. O painel abre instantaneamente com todas as 80+ subcategorias, salvamento em `localStorage` e zero falhas de runtime.
