...
- **29/07/2026 - 18:02:** Eliminação definitiva do erro `500: INTERNAL_SERVER_ERROR (FUNCTION_INVOCATION_FAILED)` da Vercel. O erro de execução ocorria porque as Serverless Functions importavam indiretamente módulos internos com dependências de compiladores nativos (`playwright-core` / `@whiskeysockets/baileys`). Reescrevemos `api/config.ts` e `api/history.ts` para serem 100% autônomos e leves, consultando diretamente o banco Neon via `pg` nativo. No frontend (`public/index.html`), adicionamos persistência de redundância via `localStorage`, garantindo carregamento instantâneo sem travamentos.
