...
- **29/07/2026 - 17:30:** Correção da exibição e persistência dos filtros e categorias na nuvem (Vercel). Como a Vercel não possui sistema de arquivos gravável para ler o `.env`, foi criada a tabela `app_settings` no **Neon PostgreSQL** com semente automática (`seed`) de todas as 21 subcategorias padrão. Atualizada a API (`src/server.ts` e `src/config/settings.ts`) para ler/salvar os filtros e subcategorias ativas diretamente no banco Neon, garantindo que o painel exiba sempre todas as opções e persista qualquer alteração feita pelo usuário no Dashboard sem depender do `.env` local.
