...
- **29/07/2026 - 17:34:** Expansão completa das **80+ subcategorias** da árvore em cascata de produtos do Mercado Livre (incluindo Celulares, Informática, Games, Eletrodomésticos, Ferramentas, Beleza, Suplementos, Esportes, Moda, Bebês, Automotivo e Pet Shop). Adicionada a função `getAllSubCategoryIds()` no frontend (`public/index.html`) e atualizado `DEFAULT_CATEGORIES` em `src/config/settings.ts` para que, ao carregar a página na nuvem, todas as subcategorias venham **100% pré-selecionadas e visíveis na tela**.
