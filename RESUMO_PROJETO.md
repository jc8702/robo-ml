...
- **29/07/2026 - 18:15:** Correção cirúrgica da Vercel. Identificado que a presença da linha `"main": "dist/index.js"` no `package.json` fazia o motor da Vercel auto-detectar o arquivo `dist/index.js` (código do robô com Playwright e Baileys) como o handler padrão da rota `/`, causando o erro `FUNCTION_INVOCATION_FAILED`. A propriedade `"main"` foi removida do `package.json` e adicionada a regra estática explícita de `rewrites` no `vercel.json` forçando a rota `/` a entregar o `index.html`.
