...
- **29/07/2026 - 17:45:** Correção da falha de Build Docker no Render. O erro `Exited with status 1` ocorria durante o download manual de bibliotecas Debian via `apt-get` no `node:20-slim`. O `Dockerfile` foi atualizado para a imagem oficial da Microsoft (`mcr.microsoft.com/playwright:v1.49.0-noble`), que já vem com o Chromium, Node.js e todas as dependências gráficas nativas do Linux 100% pré-instaladas. Deploy enviado para o GitHub para novo build automático no Render.
