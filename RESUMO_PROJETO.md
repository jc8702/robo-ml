...
- **29/07/2026 - 17:41:** Auditoria profunda e correção definitiva da rota de API na Vercel. Identificado que a Vercel Serverless Function não conseguia responder ao endpoint `/api/config` porque o arquivo `src/server.ts` executava apenas `server.listen()` sem exportar a função de handler padrão (`export default handleRequest`). Atualizado `src/server.ts` para exportar o handler serverless compatível com o `@vercel/node` e adicionada a string de conexão do **Neon PostgreSQL** como fallback direto no `getDbPool()` em `src/db/index.ts`. Deploy recompilado e publicado no GitHub.
